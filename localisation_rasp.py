...
class ImageRecognitionGUI(QWidget):
    ...
    def excel_training_set(self):     # Extract Mean Feature Vector and Variance from Excel file and Display on GUI for each training set
        ...
        for class_name in class_list:
            # Using folder names as category names
            excel_file_path = os.path.join("/home/jonx/GUI", f'{class_name}_mean_variance_data.xlsx') 
    ...
    def select_new_images(self):    # Display each image and filename on GUI
        ...
        folder_name = "/home/jonx/GUI/split_images"
    ...
    def take_image(self):   # Take 1 Image and save as image1.jpg (work for Arducam 64MP)
        # Initialize the camera
        picam2 = Picamera2()
        preview_config = picam2.create_preview_configuration(main={"size": (3840, 900)})
        ...
        flipped_image = cv2.flip(image1,-1)
        cv2.imwrite("image1.jpg",flipped_image)


    def splitting_image(self, rows, cols):    # Split Images and move the images to split_images folder 

        folder_path = "/home/jonx/GUI/split_images"
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)

        source_dir = "/home/jonx/GUI"
        destination_dir = "/home/jonx/GUI/split_images"
